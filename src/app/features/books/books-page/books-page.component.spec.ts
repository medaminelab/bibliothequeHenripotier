...
describe('BooksPageComponent', () => {
  ...
  it('should call the booksService.getBooks() method on the initialisation and get the books',
    inject([BooksService],
      (booksService: BooksService) => {

        // Spy on the getBooks method of books service and mock the returned value
        const booksServiceSpy = spyOn(booksService, 'getBooks').and.returnValue(new Observable(subscriber => {
          subscriber.next(booksMock);
        }));

        // s'assurer que la méthode n'a pas encore été appelé
        expect(booksServiceSpy).not.toHaveBeenCalled();

        fixture = TestBed.createComponent(BooksPageComponent);
        component = fixture.componentInstance;
        fixture.detectChanges();

        // s'assurer que la méthode getBooks du booksService a été appélé après l'initialisation du composant
        expect(booksServiceSpy).toHaveBeenCalled();
        expect(component.books).toEqual(booksMock);
      }));

  it('should call the booksService.freeSearch method on the change of the search field of the page',
    async(inject([BooksService],
      (booksService: BooksService) => {
        /*** mock getBooksMethod to get book and initilise the component page ***/
        spyOn(booksService, 'getBooks').and.returnValue(new Observable(subscriber => {
          subscriber.next(booksMock);
        }));
        ...
        // Spy on the getBooks method of books service and mock the returned value
        const booksServiceSpy = spyOn(booksService, 'freeSearch').and.returnValue(booksMock);

        /*** simuler la recherche effectué par l'utilisateur ***/
        const compiledInputSearchField = fixture.debugElement.nativeElement.querySelector('input');
        compiledInputSearchField.value = 'test';
        compiledInputSearchField.dispatchEvent(new Event('input'));
        compiledInputSearchField.dispatchEvent(new Event('change'));
        fixture.detectChanges();

        // s'assurer que la méthode freeSearch du service a été appelé et que le résultat a été bien réccupéré
        fixture.whenStable().then(() => {
          expect(booksServiceSpy).toHaveBeenCalled();
          expect(component.filteredBooks).toEqual(booksMock);
          booksServiceSpy.calls.reset();
        })

        // s'assurer que tous les livres s'affichent et la méthode freeSearch du service  n'est pas appelé quand l'utilisateur annule la recherche
        compiledInputSearchField.value = null;
        compiledInputSearchField.dispatchEvent(new Event('input'));
        compiledInputSearchField.dispatchEvent(new Event('change'));
        fixture.detectChanges();
        fixture.whenStable().then(() => {
  ...
  it('should not call the booksService.freeSearch method when the user delete the search',
    async(inject([BooksService],
      (booksService: BooksService) => {
        /*** mock getBooksMethod to get book and initilise the component page ***/
        spyOn(booksService, 'getBooks').and.returnValue(new Observable(subscriber => {
