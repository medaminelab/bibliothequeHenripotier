...
describe('Service: Books', () => {
  ...
  it('should call the backend service to get the list of books', fakeAsync(inject([HttpTestingController, BooksService],
    (backend: HttpTestingController, service: BooksService) => {

      /*** Appeler service Backend pour réccupérer les livres */
      let books: Book[];
      service.getBooks().subscribe((recievedResponce) => {
        books = recievedResponce;
      })

      /*** Tester l'appel du web service du Backend ***/
      const urlBooks = AppConfig.backendURL + '/books';
      const requestWrapper = backend.expectOne({ url: urlBooks });
      expect(requestWrapper.request.method).toBe('GET');

      /*** mocker la réponse du service Backend ***/
      requestWrapper.flush(booksMock);
  ...
  it('should perform a free search', inject([BooksService],
    (service: BooksService) => {
      let pattern: string;
      let filteredBooks: Book[];
      /*** Tester avec un pattern qui n'existe pas dans tous les livres  ***/
      pattern = 'testtesttest';
      filteredBooks = service.freeSearch(pattern, booksMock);
      expect(filteredBooks.length).toBe(0);

      /*** Tester avec un pattern sur les titres des livre  ***/
      pattern = 'Henri Potier et la Chambre des secrets';
      filteredBooks = service.freeSearch(pattern, booksMock);
      expect(filteredBooks.length).toBe(1);
      expect(filteredBooks[0]).toEqual(booksMock[1]);

      /*** Tester avec un pattern sur les prix des livres  ***/
      pattern = '30,00';
      filteredBooks = service.freeSearch(pattern, booksMock);
      expect(filteredBooks.length).toBe(2);
      expect(filteredBooks[0]).toEqual(booksMock[1]);
      expect(filteredBooks[1]).toEqual(booksMock[2]);

      /*** Tester avec un pattern sur les synopsys des livres  ***/
      pattern = "Juste avant d'assister à la coupe du Monde de Quidditch opposant les équipes d'Irlande et de Bulgarie, Henri Potier fait un rêve étrange dans lequel il est témoin du meurtre d'un vieux jardinier moldu par Voldemort, alors que le jardinier surprenait une conversation au sujet de Henri.";
      filteredBooks = service.freeSearch(pattern, booksMock);
      expect(filteredBooks.length).toBe(1);
      expect(filteredBooks[0]).toEqual(booksMock[3]);
    }));
